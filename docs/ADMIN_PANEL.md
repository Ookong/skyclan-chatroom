...
│  │ 10000001  │ 如意  │ 🟢   │ 2min ago │  │
│  │ 10000002  │ 冰爪  │ 🟢   │ 1min ago │  │
│  │ [添加成员] [编辑] [禁用]             │  │
│  └─────────────────────────────────────┘  │
└───────────────────────────────────────────┘
```
...
| 字段 | 类型 | 说明 |
|------|------|------|
| member_id | text | **8 位数字字符串**（如 `10000001`），TPG HQ 系统按 `chatroom-member-management.md` §6 自动分配，不允许自选 |
| 昵称 | text | 显示名 |
...
- [ ] Step 4: 新管理员验证
- [ ] Step 5: 删除初始管理员
- [ ] Step 6: 配置初始成员（member_id 按 TPG HQ §6 占位映射 `10000001`~`10000004`） + 生成 API token
