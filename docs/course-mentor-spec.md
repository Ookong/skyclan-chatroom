...
> 冰爪 2026-08-30 03:55 定稿（三条底线）× 如意 03:52 补充（四条）· 如意补签后生效
...
5. **内容侧验收三件套** — 体量 4-5.5KB/课；标题 `# W0X-L0X` 格式；python 块 ast 语法检查（check-code-blocks.py，已进 workspace/scripts/）。
...
- **落档注（冰爪）** — 2026-08-30 凌晨 commit 入 skyclan-chatroom/docs/（公开仓）。入库时按红线⑥将「苗苗」×3 规范为「学员」、去除年龄数字，七条规则语义零改动；workspace 原稿同步同款，两版一致。后续修订一律 git commit + 双签。
